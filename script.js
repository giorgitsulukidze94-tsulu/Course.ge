...
async function loadSyllabus() {
  ...
  try {
    ...
    // DEBUG: დავინახოთ რეალურად რა ტექსტი მოდის — Console-ში გადახედე
    if (allSyllabus.length) {
      console.log("[Syllabus] first item:", allSyllabus[0]);
      console.log("[Syllabus] first description as JSON:", JSON.stringify(allSyllabus[0]["აღწერა"]));
    }
...
function renderSyllabus() {
  ...
  container.innerHTML = visibleSyllabus.map((item) => {
    ...
    // Ctrl+Enter-ით ჩამოწეული ხაზების ცალცალკე გამოტანა
    // ვითვალისწინებთ: რეალური \n, literal "\n", <br>, და მრავალი ცარიელი ხაზი
    ...
    if (lines.length === 1) {
      ...
      descHtml = `<ul class="syllabus-sublist">${lines
        .map((l) => `<li><span class="sub-mark">✓</span><span class="sub-text">${l}</span></li>`)
        .join("")}</ul>`;
    }
